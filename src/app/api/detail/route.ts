...
export async function POST(req: NextRequest) {
  try {
    ...
    const [siteRaw, videoRaw, report] = await Promise.all([
      bingSearch(topic, 6).catch(() => [] as Link[]),
      bingSearch(`${topic} site:bilibili.com`, 12).catch(() => [] as Link[]),
      genReport(topic),
    ]);

    // 文章：排除视频站，取前 4 条
    let sites = siteRaw.filter((s) => !isVideoUrl(s.url)).slice(0, 4);
    // 视频：只保留真实视频页链接，取前 4 条
    let videos: Link[] = videoRaw.filter((v) => isVideoUrl(v.url)).slice(0, 4);

    // 兜底：抓取失败时退回搜索页链接，保证不空
    if (sites.length === 0) {
      sites = [
