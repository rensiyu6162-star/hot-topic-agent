...
- ✅ 用【排序】表达相关度，而不是删掉弱相关：按与「${domain}」的相关度【从高到低】排列——最贴合、最典型的排最前面并在标题前加 ⭐；关系较弱、只是沾边的排在后面。相关度强弱靠位置先后体现，绝不要因为"不够典型/只是边缘相关"就把它剔除。
- ⚠️【禁止输出逐条分析过程】不要写"关键词判断""XX属于XX领域→保留""XX边缘、更偏娱乐"这类逐条判断/分析说明文字，也不要解释你为什么保留或排除某条。直接给出【排好序的结果列表】即可，不要任何前置的分析段落。
...
- 格式示例：1. ⭐ 某条最贴合${domainList[0] || "该领域"}的热点 【${domainList[0] || "该领域"}】${glossaryBlock}`
...
- 用户说"抓热点"时，依次调用各平台的 fetch_hot_topics${
    domain ? "；抓完后按与锁定领域的相关度从高到低排序展示，沾边的也保留（只丢完全无关的），不要写逐条分析过程" : ""
  }
