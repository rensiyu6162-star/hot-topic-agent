...
import { retrieveKnowledge, formatKnowledge, KB_CATEGORIES } from "../../../lib/rag";
import {
      ...
      try {
        ...
        if (tq) {
          ...
          if (factLines.length) {
            ...
            factBlock = `\n\n【事实依据·必须遵守】以下是该话题的联网核实资料。脚本中涉及具体人物/战队/公司/作品的身份、归属、头衔、数据、近期动态时，【只能】写下面资料支撑的内容；资料与你的记忆冲突时以资料为准；资料没有的细节宁可不写，【严禁】编造或张冠李戴。时效基准：今天是 ${new Date().toISOString().slice(0, 10)}，"现状/年龄/头衔/近期动态"一律以资料中【最新进展】为准，旧状态只能写成过去式背景，严禁把已被新进展取代的旧状态当现状；人物当前年龄资料里有出生日期才写具体年龄，没有就不写具体年龄数字：\n${factLines.join("\n")}`;
          }
        }
      } catch (e) {
...
1. 开头3秒：直接甩判断或反常识断言，第一句≤20字。严禁自我介绍、"今天聊聊"、"先问大家一个问题"。
...
要求：
1. 第一句必须是事件里具体的事实、原话或数字，整句≤20字、3秒出头念完，背景第二句再补；严禁自我介绍与"今天给大家聊聊"；
2. 结构：钩子（1句）→ 价值主体（占七成，按信息点推进，两三句一个转折或新信息，${
            reqSec >= 85 ? "约一半处用一个资料里真实有的转折/加码做二次钩子，" : ""
          }不要平铺）→ 收束（只保留一个动作，取自资料真实争议/反差/悬念，没有争议就用事实收尾）；
${INFO_NARRATIVE_RULE}
4. 【事实纪律】稿中所有具体事实（数字/价格/日期/人名/机构名/引语/现场反应）必须能在上面资料里找到原句，资料没写的一个字都不许补；禁止脑补现场细节、网友反应、精确数字和历史对比；时态不许反——资料里"将于/刚开始/进行中"的事件绝不能写成"已结束/刚结束/落幕"，按资料日期推断阶段，没说结束就不许写结束；引语不许造假——引号或冒号后的直接引语必须是资料原句，只有大意时写成转述（"他表示"），禁止给转述配引号、禁止替人物造句；
